  ...
  this.config = _.extend(
    {
      ...
      defaultRecentBuilds:
        parseInt(process.env.DEFAULT_RECENT_BUILDS || '1', 10) || 1,
    },
    config
  );
...
Runner.prototype = {
  ...
  attach: function () {
    ...
    this.io.on('job.cancelled', function (id) {
      ...
      if (!self.callbackMap[id]) {
        debug(
          'Job cancelled, but no callback found. The queue is probably broken.'
        );
        return self.queue.process && self.queue.process();
      }
      self.callbackMap[id]();
    ...
    this.io.on('plugin.**', function () {
      self.emitter.emit.apply(
        self.emitter,
        [this.event].concat([].slice.call(arguments))
      );
    });
  },
  ...
  queueJob: function (job, config) {
    ...
    this.emitter.emit('browser.update', job.project.name, 'job.status.queued', [
      job._id,
      now,
    ]);
    debug(
      `[runner:${this.id}] Queued new job. Project: ${job.project.name} Job ID: ${job._id}`
    );
  },
  ...
  plugins: function (user, config, job, dirs, done) {
    ...
    var tasks = [
      function (next) {
        ...
        plugin.init(
          dirs,
          user.account(job.project.provider).config,
          job.providerConfig,
          job,
          finished
        );
    ...
    var anyPluginFailed = config.plugins.some(function (plugin) {
      if (false === plugin.enabled) {
        debug(
          `[runner:${self.id}] Found a disabled plugin: ${plugin.id} Project: ${job.project.name} Job ID: ${job._id}`
        );
        return;
      }
      if (!self.extensions.job[plugin.id]) {
        debug(
          `[runner:${self.id}] Error: Plugin not found ${plugin.id} Project: ${job.project.name} Job ID: ${job._id}`
        );
      ...
      tasks.push(function (next) {
        ...
        switch (fn.length) {
          ...
          default:
            return fn(
              user.jobplugins && user.jobplugins[plugin.id],
              plugin.config,
              job,
              context,
              cb
  ...
  processJob: function (job, config, next) {
    ...
    if (recentBuilds > 1) {
      debug(
        `Preserving the most recent ${recentBuilds} builds within ${branchBase}.`
      );
    }
    ...
    function jobDirsReady(err, dirs) {
      ...
      debug(
        `[runner:${self.id}] Job started. Project: ${job.project.name} Job ID: ${job._id}`
      );
      debug('Initializing plugins...');
      self.plugins(job.project.creator, config, job, dirs, function (
        err,
        workers
      ) {
        if (err) {
        ...
        self.config.processJob(
          ...
          function (err) {
            ...
            self.emitter.emit('job.done', jobdata);
            debug(
              `[runner:${self.id}] Job done without error. Project: ${job.project.name} Job ID: ${job._id}`
            );
            next();
          }
        );
